...
def hand_motion(prev_kp, curr_kp):
    # Grab all left and right hand keypoints (indices 132 to the end)
    prev_hands = prev_kp[132:]
    curr_hands = curr_kp[132:]

    # Create a mask to only calculate motion where hands are actually detected
    # (ignores the 0.0 values injected when MediaPipe loses tracking)
    mask = (prev_hands != 0.0) & (curr_hands != 0.0)
    
    if not np.any(mask):
        return 0.0  # No valid hand data to compare

    # Calculate average movement across all valid hand points
...
class ASLWordProcessor(VideoProcessorBase):
    ...
    def __init__(self, model, labels, show_debug=False):
        ...
        self.idle_counter = 0
        self.idle_patience = 0  # Added for state machine patience
    ...
    def recv(self, frame: av.VideoFrame):
        ...
        if self.state == "IDLE":
            ...
            # End signing if motion is low for a few consecutive frames
            if avg_motion < END_THRESH and len(self.sign_buffer) < self.seq_len:
                self.idle_patience += 1
                if self.idle_patience >= 5: # Wait 5 frames before giving up
                    self.state = "IDLE"
                    self.sign_buffer = []
                    self.vote_buffer.clear()
                    self.idle_patience = 0
            else:
                self.idle_patience = 0 # Reset patience if hands start moving again
