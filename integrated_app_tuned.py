...
RAW_FEATURES = 258
WORD_SEQ_LEN = 30
WORD_CAPTURE_FRAMES = 40
WORD_TRIM_START = 4
WORD_EXPECTED_FEATURES = RAW_FEATURES * 2  # normalized positions + velocity

# More realistic thresholds for the current 99-word model.
HARD_ACCEPT_CONF = 0.46
HARD_ACCEPT_MARGIN = 0.08
MAYBE_ACCEPT_CONF = 0.34
MAYBE_ACCEPT_MARGIN = 0.06

POSE_SIZE = 33 * 4
HAND_SIZE = 21 * 3
WINDOW_SLICES = ((4, 34), (6, 36), (8, 38))
...
def build_window_batches(capture_buffer):
    raw = np.asarray(capture_buffer, dtype=np.float32)
    if raw.ndim != 2 or raw.shape[1] != RAW_FEATURES:
        raise ValueError(f"Expected raw capture shape (T, {RAW_FEATURES}), got {raw.shape}")

    windows = []
    if len(raw) >= 38:
        for start, end in WINDOW_SLICES:
            windows.append(preprocess_word_sequence(raw[start:end]))
    else:
        windows.append(preprocess_word_sequence(raw))
...
class ASLCombinedProcessor(VideoProcessorBase):
    ...
    def predict_word(self):
        batch = build_window_batches(self.capture_buffer)
        probs_batch = self.word_model.predict(batch, verbose=0)
        ...
        if conf1 >= HARD_ACCEPT_CONF and margin >= HARD_ACCEPT_MARGIN:
            self.last_word = self.word_labels[top_idx[0]]
            self.last_word_status = "ACCEPTED"
        elif conf1 >= MAYBE_ACCEPT_CONF and margin >= MAYBE_ACCEPT_MARGIN:
            self.last_word = self.word_labels[top_idx[0]]
            self.last_word_status = "MAYBE"
        else:
            self.last_word = "UNKNOWN"
            self.last_word_status = "TRY AGAIN"
    ...
    def recv(self, frame: av.VideoFrame):
        ...
        if self.mode == "WORD":
            ...
            if self.capture_active:
                ...
                cv2.putText(
                    img,
                    "Tip: keep hands visible and sign in the middle frames",
                    (20, 80),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.65,
                    (255, 255, 255),
            ...
            if not self.capture_active:
                if self.last_word:
                    if self.last_word_status == "ACCEPTED":
                        color = (0, 255, 0)
                        prefix = "Word"
                    elif self.last_word_status == "MAYBE":
                        color = (0, 215, 255)
                        prefix = "Maybe"
                    else:
                        color = (0, 165, 255)
                    ...
                    cv2.putText(
                        img,
                        f"{prefix}: {self.last_word} ({self.last_word_conf:.2f})",
                        (20, 45),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.9,
                        color,
                    ...
                    if self.last_word_status:
                        cv2.putText(
                            img,
                            f"Status: {self.last_word_status}",
                            (20, 78),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            color,
...
if mode == "WORD":
    ...
    # st.info(
    #     "Word mode is tuned to be less strict: it averages 3 overlapping windows, "
    #     "shows MAYBE for low-confidence-but-useful guesses, and rejects only very weak clips."
    # )

webrtc_ctx = webrtc_streamer(
    key="asl-combined-tuned",
...
st.caption(
    "Tuned word mode: longer capture, middle-frame emphasis, 3-window probability averaging, "
    "softer acceptance thresholds, and MAYBE status instead of forcing UNKNOWN too often."
)
